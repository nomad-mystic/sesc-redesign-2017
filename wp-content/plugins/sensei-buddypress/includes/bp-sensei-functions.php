 ...
 * @return type int
 */
function bp_sensei_member_term_taxonomy_id( $type_name ) {
    global $wpdb;
...
function bp_sensei_members_count_by_type( $type_name ) {
    $type_id = bp_sensei_member_term_taxonomy_id( $type_name );
...
// Build Output HTML
    ...
    if( $user ) {
        ...
        foreach ( $active_courses as $course_item ) {
            ...
            // Image
            $active_html .= $woothemes_sensei->post_types->course->course_image( absint( $course_item->ID ), '100', '100', true );
...
// Build Output HTML
    ...
    if( $user ) {
        ...
        foreach ( $completed_courses as $course_item ) {
            ...
            // Image
            $complete_html .= $woothemes_sensei->post_types->course->course_image( absint( $course_item->ID ), '100', '100', true );
...
function bp_sensei_attach_forum( $group_id ) {
    ...
    if ( class_exists('bbPress') && bp_is_group_forums_active() ) {
        ...
        if ( $group->enable_forum == '1' ) {
            ...
            if ( !$forum_id ) {
                $forum_id = groups_new_group_forum( $group_id );
            } else {
                groups_update_group_forum( $group_id );
            }
...
function bp_sensei_group_activity_is_on( $key, $group_id=false, $default_true=true ){
    ...
    if( is_array( $bp_sensei_course_activity ) && isset( $bp_sensei_course_activity[$key] ) ) {
...
function buddypress_sensei_has_activities( $a, $activities ) {
    ...
    if ( ! isset( $bp->groups->current_group->id ) || ! $group_attached = $bp->groups->current_group->id ) {
       return $activities;
    }
...
function buddypress_sensei_disable_private_message( $message_info ) {
    ...
    if ( 0 == count( $message_info->recipients )  ) {
        ...
        //@todo at present buddypress trowing fatal error if we unset the all message recipient hence we can not
        ...
        $error_message = sprintf( _n( 'You can not send a private message to the teacher %s.', 'You can not send the message to the teachers %s.', $teacher_cnt, 'sensei-buddypress' ), $teacher_name_str );
