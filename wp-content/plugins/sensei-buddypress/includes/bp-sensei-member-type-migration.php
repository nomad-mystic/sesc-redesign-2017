...
class BuddyPress_Sensei_Member_Type_Migration {
	...
	function migrate_interface() {
		...
		<div class="users-migration">
			...
			<img src="<?php echo home_url() ?>/wp-admin/images/spinner.gif" alt="" class="migration-progress-ajax-spinner general-spinner" style="display: none; float: left; top: 15px; position: relative; left: 7px;">
			...
			<?php
				...
				$text_goback = ( ! empty( $_GET['goback'] ) ) ? sprintf( __( 'To go back to the previous page, <a href="%s">click here</a>.', 'sensei-buddypress' ), 'javascript:history.go(-1)' ) : '';
				$text_failures = sprintf( __( 'All done! %1$s user(s) were successfully migrated in %2$s seconds and there were %3$s failure(s). To try migration the failed users again, <a href="%4$s">click here</a>. %5$s', 'sensei-buddypress' ), "' + bps_successes + '", "' + bps_totaltime + '", "' + bps_errors + '", esc_url( wp_nonce_url( admin_url( 'tools.php?page=sensei-buddypress&goback=1' ), 'sensei-buddypress' ) . '&ids=' ) . "' + bps_failedlist + '", $text_goback );
				$text_nofailures = sprintf( __( 'All done! %1$s user(s) were successfully migrated in %2$s seconds and there were 0 failures. %3$s', 'sensei-buddypress' ), "' + bps_successes + '", "' + bps_totaltime + '", $text_goback );
				?>


				<noscript><p><em><?php _e( 'You must enable Javascript in order to proceed!', 'sensei-buddypress' ) ?></em></p></noscript>
				...
				<p>
					<input type="button" class="button hide-if-no-js" name="users-migration-" id="users-migration-stop" value="<?php _e( 'Abort Migration', 'sensei-buddypress' ) ?>" />
				</p>
				...
				<p>
					...
					<?php printf( __( 'User Migration Failures: %s', 'sensei-buddypress' ), '<span id="users-migration-debug-failurecount">0</span>' ); ?>
				</p>
							...
							$.ajax({
								...
								success: function( response ) {
									if ( response !== Object( response ) || ( typeof response.success === "undefined" && typeof response.error === "undefined" ) ) {
										...
										response.error = "<?php printf( esc_js( __( 'The migrate request was abnormally terminated (ID %s). This is likely due to the user exceeding available memory or some other type of fatal error.', 'sensei-buddypress' ) ), '" + id + "' ); ?>";
									}
	...
	function die_json_error_msg( $id, $message ) {
		die( json_encode( array( 'error' => sprintf( __( '&quot;%1$s&quot; (ID %2$s) failed to resize. The error message was: %3$s', 'sensei-buddypress' ), esc_html( get_the_title( $id ) ), $id, $message ) ) ) );
	}
